...
def test_state_dim_includes_ftmo_feats():
    env = _env()
    # lkbk*F + N_POSITION_FEATS + N_FTMO_FEATS
    assert env.state_dim == env.lkbk * env.F + N_POSITION_FEATS + N_FTMO_FEATS
    s = env.reset()
    assert s.shape == (env.B, env.state_dim)


def _ftmo_slice(env, state):
    """The last N_FTMO_FEATS columns are the v2 target/risk-aware features in the
    documented order: target, maxdd, difficulty, progress, dd_headroom,
    day_remaining, account_log."""
    return state[:, -N_FTMO_FEATS:]
...
def test_obs_schema_version_is_v2():
    env = _env()
    assert env.obs_schema_version == OBS_SCHEMA_VERSION == 2
